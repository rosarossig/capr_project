function [lik, latents] = RW1(param, data)
    ...
    for n = 1:N
        ...
        % incorporates the "forgetting" feature
        % check if this trial is the first trial of phase 2 or phase 3
        if n ~= 1 && data.block(n) ~= data.block(n-1)
             % estimation here should decay a bit..... 
             w = w*eta; 
        end
        ...
        % sum llk - the mfit package takes in a positive summed llk, not
        % negative! 
        lik = sum(lognormpdf((data.response), rhat_vec, sqrt(sigma2)));
